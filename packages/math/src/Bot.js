...
module.exports = class Bot {
    ...
    constructor(config, labyrinth) {
        ...
        this.moveLog = [];
        this.allMovesLogs = []; // TODO dunno if it is needed

        this.sandbox = {
            console: {
                log: (...args) => {
                    const message = `BOT ${this.id} log: `;
    ...
    makeAMove(config) {
        ...
        this.allMovesLogs.push(this.moveLog);
        this.moveLogString = JSON.stringify(this.moveLog);
    }
    ...
    getStats() {
        return {
            // cumulative log from the move
            l: this.moveLogString,
            // bot id
            i: this.id,
            // bot position
            p: this.position,
